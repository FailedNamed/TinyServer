...
http_conn::LINE_STATUS http_conn::parse_line()
{
    ...
    foe(; m_checked_idx < m_read_idx; ++m_checked_idx)
    {
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    *m_version++ = '\0';
    ...
    // 只支持http/1.1
    if(strcasecmp(m_version, "http://") != 0) return BAD_REQUEST;
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if(cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        else if(*(p + 1) == '2')
        {
            ...
            // 密码错误
            else strcpy(m_url, "/loginError.html");
...
bool http_conn::write()
{
    ...
    while(true)
    {
        ...
        // 这里修改了
        if(tmp >= m_iv[0].iov_len)
        {
            m_iv[0].iov_len = 0;
            m_iv[1].iov_base = m_file_address + (tmp - m_iv[0].iov_len);
            m_iv[1].iov_len = bytes_to_send;
        }
        else
        {
            m_iv[0].iov_len = m_iv[0].iov_len - tmp;
            m_iv[0].iov_base = m_write_buf + tmp;
        }
